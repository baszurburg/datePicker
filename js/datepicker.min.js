...
/*@cc_on
                ...
                th.unselectable = "on";
                /*@end@*/
return th;}function createThAndButton(tr,obj){for(var i=0,details;details=obj[i];i++){var th=createTH(details);tr.appendChild(th);var but=document.createElement("span");but.className=details.className;but.id=o.id+details.id;but.appendChild(document.createTextNode(details.text||o.nbsp));but.title=details.title||"";if(o.win32){th.unselectable=but.unselectable="on";}th.appendChild(but);}}function createSingleThAndButton(details){var th=createTH(details);var but=document.createElement("span");but.className=details.className;but.id=o.id+details.id;but.appendChild(document.createTextNode(details.text||o.nbsp));but.title=details.title||"";
/*@cc_on
                 /*@if(@_win32)
                 th.unselectable = but.unselectable = "on";
                 /*@end@*/
th.appendChild(but);return th;}function createMonthPicker(el){var select=document.createElement("select"),option,i;select.setAttribute("name","month-picker");select.setAttribute("class","month-picker");select.setAttribute("id",o.id+"-month-picker");for(i=0;i<12;i++){option=document.createElement("option");option.setAttribute("value",i);option.innerHTML=getMonthTranslation(i,false);select.appendChild(option);}el.appendChild(select);}function createYearPicker(el){var select=document.createElement("select"),option,i;select.setAttribute("name","year-picker");select.setAttribute("class","year-picker");select.setAttribute("id",o.id+"-year-picker");for(i=o.yearPickerStart;i<o.yearPickerEnd+1;i++){option=document.createElement("option");option.setAttribute("value",i);option.innerHTML=i;select.appendChild(option);}el.appendChild(select);}this.div=document.createElement("div");this.div.id="fd-"+this.id;this.div.className="date-picker"+this.bespokeClass;this.div.style.visibility="hidden";this.div.style.display="none";if(this.describedBy&&document.getElementById(this.describedBy)){setARIAProperty(this.div,"describedby",this.describedBy);}if(this.labelledBy){setARIAProperty(this.div,"labelledby",this.labelledBy.id);}var tr,row,col,tableHead,tableBody,tableFoot;this.table=document.createElement("table");this.table.className="date-picker-table";this.table.onmouseover=this.onmouseover;this.table.onmouseout=this.onmouseout;this.table.onclick=this.onclick;if(this.staticPos){this.table.onmousedown=this.onmousedown;}this.div.appendChild(this.table);var dragEnabledCN=!this.dragDisabled?" drag-enabled":"";if(!this.staticPos){this.div.style.visibility="hidden";this.div.className+=dragEnabledCN;document.getElementsByTagName("body")[0].appendChild(this.div);
/*@cc_on
                @if(@_jscript_version <= 5.7)
                ...
                @end
                @*/
setARIAProperty(this.div,"hidden","true");}else{elem=document.getElementById(this.positioned?this.positioned:this.id);if(!elem){this.div=null;if(debug){throw this.positioned?"Could not locate a datePickers associated parent element with an id:"+this.positioned:"Could not locate a datePickers associated input with an id:"+this.id;}return;}this.div.className+=" static-datepicker";if(this.positioned){elem.appendChild(this.div);}else{elem.parentNode.insertBefore(this.div,elem.nextSibling);}if(this.hideInput){for(var elemID in this.formElements){elem=document.getElementById(elemID);if(elem){elem.className+=" fd-hidden-input";}}}setTimeout(this.resizeInlineDiv,300);}setARIARole(this.div,"application");setARIARole(this.table,"grid");if(this.statusFormat){tableFoot=document.createElement("tfoot");this.table.appendChild(tableFoot);tr=document.createElement("tr");tr.className="date-picker-tfoot";tableFoot.appendChild(tr);this.statusBar=createTH({thClassName:"date-picker-statusbar"+dragEnabledCN,colspan:this.showWeeks?8:7});tr.appendChild(this.statusBar);this.updateStatus();}tableHead=document.createElement("thead");tableHead.className="date-picker-thead";this.table.appendChild(tableHead);tr=document.createElement("tr");setARIARole(tr,"presentation");tableHead.appendChild(tr);this.titleBar=createTH({thClassName:"date-picker-title"+dragEnabledCN,colspan:this.showWeeks?8:7});tr.appendChild(this.titleBar);tr=null;if(options.showMonthPicker){createMonthPicker(this.titleBar);}else{var span=document.createElement("span");span.appendChild(document.createTextNode(nbsp));span.className="month-display"+dragEnabledCN;this.titleBar.appendChild(span);}if(options.showYearPicker){createYearPicker(this.titleBar);}else{span=document.createElement("span");span.appendChild(document.createTextNode(nbsp));span.className="year-display"+dragEnabledCN;this.titleBar.appendChild(span);}span=null;tr=document.createElement("tr");setARIARole(tr,"presentation");tableHead.appendChild(tr);var th,thToday;th=createSingleThAndButton({className:"prev-but prev-year",id:"-prev-year-but",text:"\u00AB",title:getTitleTranslation(2)});tr.appendChild(th);th=createSingleThAndButton({className:"prev-but prev-month",id:"-prev-month-but",text:"\u2039",title:getTitleTranslation(0)});tr.appendChild(th);thToday=createSingleThAndButton({colspan:this.showWeeks?4:3,className:"today-but",id:"-today-but",text:getTitleTranslation(4)});tr.appendChild(thToday);th=createSingleThAndButton({className:"next-but next-month",id:"-next-month-but",text:"\u203A",title:getTitleTranslation(1)});tr.appendChild(th);th=createSingleThAndButton({className:"next-but next-year",id:"-next-year-but",text:"\u00BB",title:getTitleTranslation(3)});tr.appendChild(th);tableBody=document.createElement("tbody");this.table.appendChild(tableBody);var colspanTotal=this.showWeeks?8:7,colOffset=this.showWeeks?0:-1,but,abbr,formElemId,formElem;for(var rows=0;rows<7;rows++){row=document.createElement("tr");if(rows!=0){setARIARole(row,"row");tableBody.appendChild(row);}else{tableHead.appendChild(row);}for(var cols=0;cols<colspanTotal;cols++){if(rows===0||(this.showWeeks&&cols===0)){col=document.createElement("th");}else{col=document.createElement("td");setARIAProperty(col,"describedby",this.id+"-col-"+cols+(this.showWeeks?" "+this.id+"-row-"+rows:""));setARIAProperty(col,"selected","false");}
/*@cc_on@*/
/*@if(@_win32)
                    col.unselectable = "on";
                    /*@end@*/
